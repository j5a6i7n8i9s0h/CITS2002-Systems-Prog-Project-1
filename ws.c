...
/*
    A function to print out the struct when it has finished processing data.
*/
void printList( WifiList w[])
{
    for(int i = 0; i < macNote; i++) {
        printf("%s\t%d\n",w[i].macaddress ,w[i].packetsize);
    }
...
int findoui(char threemac[],char *mac[],int line) {
    for(int i=0;i<line;i++){
        if(mystrcmp(threemac,mac[i])) return i;
    }
    return -1;
}
/*
    ...
    if(strcmp(fileoui,"FILENA")==0){
        ...
        switch (pid = fork()) {
            ...
            case 0:                 //new child process
                printf("\n");
                execl("/usr/bin/sort", "/usr/bin/sort","-t", "\t", "-k2,2rn", "-k1,1", "result.txt", (char*)NULL);
                exit(EXIT_SUCCESS);
                break;
      ...
      // line holds notes the number of vendors by fileoui.
        ...
        while(fgets(buff,BUFSIZ,ven)!=NULL){
            ...
            if(venIndex>90){
                fprintf(stderr,"Vendor name is too long at line %i for file %s",line+1,fileoui);
                exit(EXIT_FAILURE);
            }
            ...
            switch (pid = fork()) {
                ...
                case 0:                 //new child process
                    execl("/usr/bin/sort", "/usr/bin/sort","-t", "\t", "-k3,3rn", "-k2,2", "result.txt", (char*)NULL);
                    exit(EXIT_SUCCESS);
                    break;
...
void transmit(char filename[],char fileoui[]){
    ...
    if(strcmp(fileoui,"FILENA")==0){
      ...
      // line holds notes the number of vendors by fileoui.
        int line=0;
        while(fgets(buff,1000,ven)!=NULL){
            // holds mac address ff:ff:ff for each line.
            char hex[10];
            int venIndex= strlen(buff)- 9;
            if(venIndex>90){
            ...
            char buff2[1000];
                // index to note position of mac address and vendor in arrays macVen and Vendors.
            int index;
            while(fgets(buff2,1000,file)!=NULL){
                int noOftabs=0;
                int len = strlen(buff2);
                int k=0;
                // macpos serves same purpose as before and holds index. Everything is same here on out till finding value of index
...
int main(int argc,char* argv[]){
    if(argc==3){
        ...
        fprintf(stderr,"Provide input as ./program what packets ouifile(optional)"  );
    }
    return 0;
}
